...
export function weekLabel(d){
  const m = mondayOf(d), s = sundayOf(d);
...
/* ===========================
   共用 UI/資料 輔助
   =========================== */
export function isComplete(a){
  return (a.watched?.length||0) >= a.episodes && a.episodes > 0;
}
export function badgeHTML(a){
  return `<span class="badge ${isComplete(a)?'complete':''}">觀看 ${a.watched?.length||0} / ${a.episodes}</span>`;
}
export function nextUnwatched(a){
  const set = new Set(a.watched||[]);
  for(let i=1;i<=a.episodes;i++){ if(!set.has(i)) return i; }
  return null;
}
